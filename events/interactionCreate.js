    ...
    async execute(interaction, client) {
        ...
        if (interaction.isButton()) {
                ...
                if (ticketCheck.hasTicket) {
                    try {
                        await interaction.editReply({ 
                            content: `â You already have an open ticket: ${ticketCheck.ticketChannel}. Please close your current ticket before creating a new one.`, 
                            components: []
                        });
                    } catch (error) {
            ...
            if (interaction.customId.startsWith('close_ticket_')) {
                ...
                try {
                    ...
                    const closedEmbedInTicket = new EmbedBuilder()
                        ...
                        .setDescription(`Ticket closed by ${interaction.user}.\n\nð **Transcript saved automatically**\nð **Trader points awarded**`)
                        .addFields(
            ...
            if (interaction.customId.startsWith('final_reopen_ticket_')) {
                ...
                const originalCloseButton = new ButtonBuilder().setCustomId(`close_ticket_${ticketChannel.id}`).setLabel('Close Ticket').setStyle(ButtonStyle.Danger).setEmoji('ð');
            ...
            if (interaction.customId.startsWith('mm_request_modal_')) {
                ...
                try {
                    ...
                    if (otherTraderUser.bot) { return interaction.editReply({ content: 'â You cannot create a ticket with a bot.', ephemeral: true }); } 
                    if (otherTraderUser.id === ticketCreator.id) { return interaction.editReply({ content: 'â You cannot create a ticket with yourself.', ephemeral: true }); } 
                    ...
                    if (otherTraderTicketCheck.hasTicket) {
                        return interaction.editReply({ 
                            content: `â The other trader (${otherTraderUser.tag}) already has an open ticket: ${otherTraderTicketCheck.ticketChannel}. They need to close their current ticket first.`, 
                            ephemeral: true 
                        });
                    }
                    
                    const otherTraderMMBan = currentDB.mmBans.find(ban => ban.userId === otherTraderUser.id); 
                    if (otherTraderMMBan) { return interaction.editReply({ content: `â The other trader (${otherTraderUser.tag}) is currently banned from using the middleman service. Reason: ${otherTraderMMBan.reason}`, ephemeral: true }); } 
                    ...
                    const closeButton = new ButtonBuilder().setCustomId(`close_ticket_${thread.id}`).setLabel('Close Ticket').setStyle(ButtonStyle.Danger).setEmoji('ð'); const row = new ActionRowBuilder().addComponents(closeButton); 
                    ...
                    await interaction.editReply({ content: `â Ticket created! You can find it here: ${thread}\n\n**Found trader:** ${otherTraderUser.tag}`, components: [] });
                    saveDB(client.db); 
                ...
                try {
                    ...
                    const thankYouEmbed = new EmbedBuilder().setColor('#000000').setTitle('ð Feedback Submitted!').setDescription('Thank you for your valuable feedback. It helps us improve our middleman service!').setTimestamp(); 
