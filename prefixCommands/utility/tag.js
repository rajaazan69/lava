...
module.exports = {
    ...
    async execute(message, args, client) {
        const mmRoleId = '1378701171337859112';
        const isAdmin = message.member.permissions.has(PermissionsBitField.Flags.Administrator);
        const isMM = message.member.roles.cache.has(mmRoleId);

        if (!isAdmin && !isMM) {
            return message.reply({ content: 'You must be an **Admin** or **Middleman** to use this command.' });
        }
        ...
        if (actionOrTagName === 'create') {
            ...
            if (tagName.length > 50) return message.reply({ content: 'Tag name cannot exceed 50 characters.' });
            if (['create', 'delete', 'list', 'edit'].includes(tagName.toLowerCase())) return message.reply({ content: `Tag name "${tagName}" cannot be a reserved keyword.` });
            if (!content) return message.reply(`You need to provide content for the tag!\nUsage: \`${prefixToUse}tag create <tagname> <content>\``);
            if (content.length > 1800) return message.reply({ content: 'Tag content cannot exceed 1800 characters.' });
            if (client.db.tags[tagName.toLowerCase()]) return message.reply({ content: `A tag with the name "${tagName}" already exists.` });
        ...
        if (actionOrTagName === 'delete') {
            ...
            if (!client.db.tags[tagToDelete]) return message.reply({ content: `Tag "${tagName}" not found.` });
        ...
        if (actionOrTagName === 'edit') {
            ...
            if (content.length > 1800) return message.reply({ content: 'New tag content cannot exceed 1800 characters.' });

            const tagToEdit = tagName.toLowerCase();
            if (!client.db.tags[tagToEdit]) return message.reply({ content: `Tag "${tagName}" not found.` });
        ...
        if (actionOrTagName === 'list') {
            ...
            if (tagList.length === 0) return message.reply({ content: 'There are no tags created yet.' });
        ...
        if (client.db.tags[requestedTagName]) {
            ...
            return message.reply({ content: `Tag "${requestedTagName}" not found. Use \`${prefixToUse}tag list\` to see available tags.` });
        }
    },
};